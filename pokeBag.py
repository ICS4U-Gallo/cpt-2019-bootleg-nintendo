...
def on_draw(game):
    ...
    if game.selected != None:
        ...
        arcade.draw_text(f"# {game.selected.num}", 440, 460, arcade.color.BLACK, 20)
        arcade.draw_text(f"lvl: {game.selected.lvl}", 440, 440, arcade.color.BLACK, 20)
        arcade.draw_text(f"hp: {game.selected.cur_stats[0]} / {game.selected.stats[0]}", 440, 420, arcade.color.BLACK, 20)
        arcade.draw_text(f"atk: {game.selected.stats[1]}", 440, 400, arcade.color.BLACK, 20)
        arcade.draw_text(f"def: {game.selected.stats[2]}", 440, 380, arcade.color.BLACK, 20)
        arcade.draw_text(f"spd: {game.selected.stats[3]}", 440, 360, arcade.color.BLACK, 20)
    ...
    if button == arcade.MOUSE_BUTTON_LEFT:
        ...
        if game.switch_pos:
            if game.switched:
                ...
                game.player_sprite.pokemon[search_for(game.player_sprite.pokemon, game.switch_from)], game.player_sprite.pokemon[j] = game.selected, game.switch_from
                game.switch_pos = False
                game.switched = False
