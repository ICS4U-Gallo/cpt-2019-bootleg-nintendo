...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.search_number = ""
        self.potion_amount = 0
        self.super_potion_amount = 0
    ...
    def on_draw(self):
        ...
        if self.get_pointer() - 1 != -1:
            arcade.draw_text("{}: {}".format(self.heal_list
                             [self.get_pointer() - 1].name,
                             self.heal_list[self.get_pointer() - 1].effect), 20,
                             425, arcade.color.BLACK, 20)

        arcade.draw_text("{}: {}".format(self.heal_list
                         [self.get_pointer()].name,
                         self.heal_list[self.get_pointer()].effect), 20,
                         320, arcade.color.BLACK, 30)

        if self.get_pointer() + 1 != len(self.heal_list):
            arcade.draw_text("{}: {}".format(self.heal_list
                             [self.get_pointer() + 1].name,
                             self.heal_list[self.get_pointer() + 1].effect), 20,
        ...
        arcade.draw_texture_rectangle(625, 335, 240, 240, self.heal_list
                                      [self.get_pointer()].texture)
        arcade.draw_text("Amount: {}".format(str(self.heal_list[self.get_pointer()].amount)), 465, 150, arcade.color.BLACK, 50)
    ...
    def on_key_press(self, key, key_modifiers):
        if key == arcade.key.W:
            ...
            if self.get_pointer() != len(self.heal_list) - 1:
                self.edit_pointer(self.get_pointer() + 1)
            ...
            if key == arcade.key.L and self.search_number != "":
                found = binary_search(int(self.search_number), self.heal_list)
        ...
        if key == arcade.key.Q or self.sort_menu:
            ...
            self.search_menu = False
            if key == arcade.key.L:
                self.heal_list = merge_sort(self.heal_list)
                self.sort_menu = False
            elif key == arcade.key.K:
                self.sort_menu = False
